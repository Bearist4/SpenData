...
struct TransactionsView: View {
    ...
    private var categorySummaries: [(category: String, amount: Double, count: Int)] {
        // Debug: Print all bills and their categories
        print("🔍 All bills in database:")
        bills.forEach { bill in
            print("Bill: \(bill.name ?? "unnamed"), Category: \(bill.category ?? "nil"), ID: \(bill.id ?? "nil")")
        }
        
        let groupedBills = Dictionary(grouping: bills) { $0.category ?? BillCategory.uncategorized.rawValue }
        
        // Debug: Print grouped bills
        print("\n📊 Grouped bills by category:")
        groupedBills.forEach { category, bills in
            print("Category: \(category), Count: \(bills.count)")
            bills.forEach { bill in
                print("  - \(bill.name ?? "unnamed"), ID: \(bill.id ?? "nil")")
            }
        }
    ...
    private var categoryTotals: [(category: String, amount: Double)] {
        let grouped = Dictionary(grouping: bills) { $0.category ?? BillCategory.uncategorized.rawValue }
                    ...
                    ForEach(categorySummaries, id: \.category) { summary in
                        NavigationLink(destination: CategoryDetailView(category: summary.category)) {
                            HStack {
                                ...
                                Spacer()
                                Text("$\(summary.amount, specifier: "%.2f")")
                                    .font(.headline)
                            }
                        }
                    }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Text(bill.name ?? "Unnamed Bill")
                            .font(.headline)
                        Spacer()
                        Text("$\(bill.amount / Double(bill.isShared ? bill.numberOfShares : 1), specifier: "%.2f")")
                            .font(.headline)
                    }
